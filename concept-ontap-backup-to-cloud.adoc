...
* 단일 파일 복원을 위한 개별 파일을 선택하기 위한 탐색 가능한 파일 카탈로그.
...
클라우드 백업은 인터넷에 연결되지 않은 사이트("오프라인" 또는 "다크" 사이트라고도 함)에서 로컬 사내 ONTAP 시스템의 볼륨 데이터를 로컬 NetApp StorageGRID 시스템으로 백업하는 데 사용할 수 있습니다. 이 경우 어두운 사이트에 Cloud Manager Connector(최소 버전 3.9.17)를 배포해야 합니다. 을 참조하십시오 link:task-backup-onprem-private-cloud.html["사내 ONTAP 데이터를 StorageGRID에 백업"] 를 참조하십시오.
...
+
인스턴스는 Connector와 동일한 서브넷에 상주하며 복원할 개별 파일을 찾기 위해 백업 파일을 탐색할 때만 실행됩니다. 비용 절감을 위해 사용하지 않을 때는 인스턴스가 해제됩니다.

+
** AWS에서 복구 인스턴스는 에서 실행됩니다 https://aws.amazon.com/ec2/instance-types/m5/["m5n.xlarge 인스턴스"^] CPU 4개, 16GiB 메모리 및 EBS 전용 인스턴스 스토리지 운영 체제 이미지는 Amazon Linux 2입니다.
+
m5n.xlarge 인스턴스를 사용할 수 없는 지역에서는 대신 M5.xlarge 인스턴스에서 Restore가 실행됩니다.

** Azure에서 Restore virtual machine은 에서 실행됩니다 https://docs.microsoft.com/en-us/azure/virtual-machines/dv3-dsv3-series#dsv3-series["Standard_D4s_v3 VM"^] CPU 4개, 16GiB 메모리 및 32GiB 디스크 사용 운영 체제 이미지는 CentOS 7.5)입니다.
+
...
* 검색 및 복원을 사용한 볼륨 또는 파일 복원의 경우 Amazon Athena 및 AWS Glue 리소스가 필요합니다. 볼륨 및 파일을 복원할 때 이러한 리소스에 비용이 연결됩니다. 검색 및 복원은 현재 AWS A3에서만 지원됩니다.
* 아카이브 스토리지로 이동한 백업 파일(ONTAP 9.10.1 이상을 사용하는 경우 AWS 및 Azure에서 지원)에서 볼륨 데이터를 복구해야 하는 경우 클라우드 공급자로부터 GB당 추가 검색 비용 및 요청당 요금이 부과됩니다.
...
AWS Marketplace에서 2개의 연간 계약을 사용할 수 있습니다.
...
* 12, 24 또는 36개월 간의 연간 계약을 사용하여 Cloud Volumes ONTAP 및 클라우드 백업을 번들로 제공할 수 있는 프로페셔널 패키지 이 옵션을 사용하면 온프레미스 데이터를 백업할 수 없습니다.
+
...
* 인터넷 액세스(어두운 사이트)가 없는 사이트에 Connector를 설치한 경우에는 파일 수준 복원이 지원되지 않습니다.
