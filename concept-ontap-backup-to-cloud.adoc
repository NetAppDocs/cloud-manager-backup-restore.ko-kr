...
* 검색 및 복원을 사용한 볼륨 또는 파일 복원의 경우 Amazon Athena 및 AWS Glue 리소스를 사용하여 볼륨 및 백업 파일을 검색할 수 있습니다. TiB 비용은 검색 요청에 의해 스캔된 데이터 양과 연관됩니다. 검색 및 복원은 현재 Amazon S3의 백업 파일에서 데이터를 복원할 때만 지원됩니다.
