...
+
image:screenshot_auto_backup_k8s.png["클러스터의 모든 향후 볼륨에 할당할 백업 정책을 선택하는 방법을 보여 주는 스크린샷"]
...
. 백업 설정 페이지에서 _ 을(를) 클릭합니다 image:screenshot_horizontal_more_button.gif["추가 아이콘"] 볼륨이 있는 작업 환경의 경우 * 영구 볼륨 관리 * 를 선택합니다.
+
image:screenshot_backup_manage_volumes_k8s.png["백업 설정 페이지의 영구 볼륨 관리 단추를 보여 주는 스크린샷"]
